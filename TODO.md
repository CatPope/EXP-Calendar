...
- [ ] **페르소나 구조화 필드 백엔드 반영**: 단일 `definition` 필드에서 구조화된 필드(`persona_name`, `tone`, `history`, `frequent_thought`)로 백엔드 API 및 DB 스키마 리팩토링
- [ ] **페르소나 편집 무료화**: SRS v1.4의 무료 편집 요구사항(`FR-SOC-02`)에 맞춰 백엔드의 캐릭터 설정권(`persona_tokens`) 소모 로직 제거 및 관련 API 수정
- [ ] **"소환(가챠)" → "스킨 뽑기" 명칭 일원화** (FR-SUMMON, UI-09, 와프 07): 프론트 라우트(`summon/` 유지 또는 `skin-draw/`)·라벨·아이콘 일괄. API 경로 변경은 호환 유지 검토 후 결정
- [ ] **상태 메시지(대사) 편집** (FR-STAT-06, 와프 09): 통계·등급 화면에 편집 UI + `PERSONA.status_message` 컬럼(또는 SRS 정합성에 따른 컬럼) 추가 마이그레이션
