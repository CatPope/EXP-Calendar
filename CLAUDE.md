...
**EXP Calendar** — 게이미피케이션 기반 일정 관리 PWA. Google Calendar 연동 + EXP/포인트/칭호 보상 + LLM 페르소나 + 소셜 쇼케이스. 개발 기한 2주(1주 개발 + 1주 테스트), SRS v1.4 기준 Part A~H·L 필수, I~K 권장.
...
- **세부 구현은 담당자가 직접 판단**하되, 결정 사항·진행 상태·막힘 사유는 TODO.md에 기록한다 (진행 중 `[~]`, 완료 `[x]`, 막힘 `[!]` + 한 줄 사유).
- TODO와 어긋나는 결정이 필요해지면 **TODO.md를 함께 수정**하고 그 사유를 PR 본문/커밋 메시지에 남긴다.
...
- **이전 버전 SRS(`Legacy/requirements_ieee830_v1.2.md`, `Legacy/requirements_ieee830_v1.3.md`)는 절대 수정 금지**. 동결 보존된 직전 버전들로, 권위 비교용이다. **현재 권위 문서는 `requirements_ieee830_v1.4.md`이며 사용자 확인 없이 임의 갱신·정정·문구 다듬기 모두 금지**. 코드가 SRS와 어긋날 때는 코드를 고치고, SRS 변경이 필요해 보이면 사용자에게 먼저 확인.
