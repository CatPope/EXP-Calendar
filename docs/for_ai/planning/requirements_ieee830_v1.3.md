...
> **v1.3 개정 요지**: UI/UX 와이어프레임(`docs/for_ai/planning/UXUI.pdf`, 11화면)에 맞춰 요구사항을 정합화한 버전이다. 원본 v1.2(`requirements_ieee830.md`)는 변경 없이 보존하며, 본 문서는 그 사본을 기반으로 수정되었다. 주요 추가: 소환(가챠) 기반 **단일 캐릭터 수집**(펫 육성 기능 없음), 통합 설정 화면, 구조화 페르소나, 등급 5단계(D~S)·스트릭 통계, 일일 퀘스트 차등 보상·보너스. 다국어·GCal 양방향 동기화·다중 테마는 **설정 화면에 노출하되 실제 동작은 V2**로 둔다. 변경 항목은 본문에 `[v1.3]` 태그로 표기한다.
...
| UI-09 `[v1.3]` | **소환(가챠) 화면**: 픽업 배너, 확률표(등급별 %), 천장 카운트, 단차/10연차 버튼, 도감(수집) 그리드를 제공한다. | 권장 |
| UI-10 `[v1.3]` | **통계·등급 화면**: 내 캐릭터 프로필(상태 메시지/대사), 주간·월간 성공률·최장 스트릭, 잔디 그래프, 성공/실패 시계열, 등급(D~S) 게이지를 제공한다. | 필수 |
...
| FR-SHOP-03 | 판매 항목: 캐릭터 커스터마이징 재화(머리 장식·이펙트·**쇼케이스 배경**), 등급 하락 방어권, 캐릭터 성격·말투 **프리셋** 아이템, **소환권**. 탭은 `커스터마이징`/`방어 아이템`/`페르소나` 3종으로 구성한다. `[v1.3]` | 필수 |
...
| FR-SOC-02 | 페르소나는 **구조화 필드**(이름 / 말투·성격 / 역사·배경 / 자주 하는 생각)로 정의하며, 이를 System Prompt(성격·칭호 이력 주입)에 주입해 사용자 입력 텍스트를 **캐릭터 말투로 변환**, 쇼케이스에 전시한다. 구조화 필드 편집 자체는 무료이고, 상점 `페르소나` 프리셋은 선택적 보조 아이템이다. `[v1.3]` | 필수 |
...
#### 3.2.9 소환 · 캐릭터 수집 `[v1.3]`

> 와이어프레임 06 화면(픽셀용사 소환)에 대응한다. **수집 대상은 단일 활성 캐릭터**이며, 다마고치식 펫 육성 기능은 제공하지 않는다.

| ID | 요구사항 | 우선순위 |
|----|----------|:--------:|
| FR-SUMMON-01 | 포인트 기반 **소환(가챠)**으로 캐릭터를 획득한다. 단차(100P)·10연차(900P)를 제공하며, 10연차 시 **RARE 이상 1개 확정**. | 권장 |
| FR-SUMMON-02 | 등급별 획득 확률을 적용한다: **COMMON 60% / RARE 28% / EPIC 9% / LEGENDARY 3%**. 픽업 배너 적용 시 해당 LEGENDARY 확률을 **2배**로 가중한다. | 권장 |
| FR-SUMMON-03 | **천장(pity)**: 90회 연속 LEGENDARY 미획득 시 다음 소환에서 LEGENDARY 1개를 확정 지급한다. | 권장 |
| FR-SUMMON-04 | 획득 캐릭터는 **도감(컬렉션)**에 누적되고, 사용자는 보유 캐릭터 중 **1개만 활성 캐릭터로 장착**한다. 중복 획득은 포인트로 환급한다. | 권장 |
| FR-SUMMON-05 | 소환 비용은 **포인트 또는 소환권**으로 지불한다. 소환권은 보상·상점을 통해 획득한다. | 권장 |
...
> 와이어프레임 05 화면(칭호 컬렉션)에 대응한다. FR-TITLE-01의 부여 대상이며, 조건은 본 v1.3에서 선정한 값이다. **구현 시 SSoT(`api_and_rules.md`)와 `backend/internal/game/engine.go`를 본 표에 맞춰 동기화해야 한다.**
...
## Appendix D: 소환 확률·천장 `[v1.3]`

> FR-SUMMON-02/03 요약. 픽업 배너 적용 시 해당 LEGENDARY 확률 2배.

| 등급 | 기본 확률 | 비고 |
|------|---------:|------|
| LEGENDARY | 3% | 천장 90회 시 확정 |
| EPIC | 9% | |
| RARE | 28% | 10연차 시 1개 이상 확정 |
| COMMON | 60% | |

- 단차 100P / 10연차 900P (포인트 또는 소환권)
- 천장: 90회 연속 LEGENDARY 미획득 시 다음 소환에서 확정
