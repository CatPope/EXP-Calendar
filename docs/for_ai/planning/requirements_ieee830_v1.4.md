...
| **K — 스킨 뽑기·캐릭터 수집** `[v1.4]` | 스킨 뽑기(단차/10연차), 확률·천장, 뽑기권, 도감, **단일 캐릭터 장착(펫 육성 없음)** | FR-SUMMON-01~05 | 권장 | Part B + D (포인트 재화) |
...
> **참고**: 휴면 계정 정책(FR-DORM-01~06)은 배치 스케줄러가 필요하며, 핵심 루프(일정→보상→칭호)와 독립적이다.
>
> **구현 상태 (2026-06-14)** — Part I 구현 완료. 구체 수치는 `docs/for_ai/spec/api_and_rules.md` 휴면/복귀 절을 권위로 한다.
>
> | 항목 | 값 |
> |------|----|
> | 휴면 전환 임계 | `last_active_at < now() - 14d` |
> | 경고 알림 시점 | 13일차 (워커가 시간 단위로 스윕) |
> | 복귀 포인트 (FR-DORM-03) | 일일 한도(200) × 14 = **2,800P**, 일일 한도 우회 1회성 |
> | 복귀 EXP 버프 (FR-DORM-04) | 7일간 ×1.5, EXP 한정 (포인트 미가산) |
> | 최초 복귀 방어권 (FR-DORM-05) | 3장. `users.dormant_returned_count = 0` 일 때만 |
> | 활동 heartbeat | 로그인 + `GET /api/me` 시 `last_active_at = now()` 갱신 |
